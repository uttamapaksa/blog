...
메뉴 페이지에서 카테고리 별로 포스트를 필터링 할 수 있게 변경했다. 서버 컴포넌트로 구현하려 했는데 필터링 기능을 위해선 필터링을 위한 클라이언트 컴포넌트를 새로 만들어야 했다. 서버 컴포넌트에서 페칭해놓은 포스트들을 props로 보냈다. 카테고리만 모아놓는 페이지는 데이터 페칭 함수가 달라서 정적 라우팅으로 별도로 만들었다. 쿼리 파라미터를 읽어 필터링을 하고, 초기화와 clean URL을 위해 `window.history.pushState()`를 사용했다. 
<br/>

#### useState 무한 렌더링 ✅ <small className="text-gray-500">24.12.25.</small>
카테고리에 따른 포스트 필터링 과정에서 발생했다. 컴포넌트 내에서 그냥 상태를 업데이트하면, 갱신 → 렌더링 → 다시 setter 호출을 해서 무한히 렌더링된다. `new Set()`의 참조값 변경 때문인 줄 알았는데, setter 호출만 하면 (상태가 바뀌지 않아서 DOM 업데이트를 건너뛰더라도) 무조건 다시 렌더링이 된단다. state는 immutable 권장이라서 새 객체를 생성하는 건 맞다. 갱신 조건식을 `useState` 초기값에 넣어서 해결했다. 이 초기값 계산식은 함수 형태로 넣어야 좋다. 계산식 자체를 넣으면 렌더링마다 (상태 업데이트는 되지 않지만) 계산이 다시 이루어진다. 그런데 같은 페이지에서 클릭 시 이미 초기화된 상태는 업데이트가 안돼서 결국  `useEffect`로 바꿨다.
<br/>
...
#### 로컬 내비게이션 Disclosure와 주소창 동기화 ⚠️ <small className="text-gray-500">24.01.01.</small>
...
#### 코드 블록 컴포넌트 ✅ <small className="text-gray-500">24.01.05.</small>
...
#### 테일윈드 동적 스타일링 미적용 ✅ <small className="text-gray-500">24.01.08.</small>
