...
카테고리에 따른 포스트 필터링 과정에서 발생했다. 컴포넌트 내에서 그냥 상태를 업데이트하면, 갱신 → 렌더링 → 다시 setter 호출을 해서 무한히 렌더링된다. `new Set()`의 참조값 변경 때문인 줄 알았는데, setter 호출만 하면 (상태가 바뀌지 않아서 DOM 업데이트를 건너뛰더라도) 무조건 다시 렌더링이 된단다. state는 immutable 권장이라서 새 객체를 생성하는 건 맞다. 갱신 조건식을 `useState` 초기값에 넣어서 해결했다. 이 초기값 계산식은 함수 형태로 넣어야 좋다. 계산식 자체를 넣으면 렌더링마다 (상태 업데이트는 되지 않지만) 계산이 다시 이루어진다. 근데 같은 페이지에서 클릭 시 이미 초기화된 상태는 업데이트가 안돼서 `useEffect`로 바꿨다.
<br/>
