...
`next-mdx-remote`의 `compileMDX` 함수를 활용해, 문자열을 넣으면 코드 블록으로 반환하는 컴포넌트를 만들었다. `compileMDX`는 비동기 함수라서 동기적으로 렌더링되는 리액트 클라이언트 컴포넌트에서는 오류가 난다. `useEffect`안에서 해야 안전하게 렌더링할 수 있다. 즉시 실행 함수로 만들어서 `useState`에 값을 넣었다. 멀티 라인이니까 백틱 3개(\`\`\`)를 넣고 이스케이프(특수 문자를 일반 문자처럼 쓰는 것)를 각각 붙여야했다.
<br/>
