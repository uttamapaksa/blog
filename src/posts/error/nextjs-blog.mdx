...
사이드바에 디스클로저 버튼으로 트리 형태의 로컬 내비게이션을 만들었다. 1) 각 노드를 클릭하면 여닫기가 되면서 페이지 내용이 변경되고 2) 주소창으로 접근했을 때도 경로의 노드들이 모두 열리도록 해야했다. 두 가지 난점이 있었다. 첫째는 Headless UI의 컴포넌트에서 여닫기 상태를 제어할 수 없었다. 'Render Props Pattern'과 'Compound Component Pattern'을 활용해 Custom UI를 만들어서 Headless UI와 유사하면서도 setter에 접근할 수 있도록 변경했다. 둘째는 URL이 바뀔 때마다 노드들이 전부 전체 리렌더링되어 상태 유지가 안되었다. `[[...slugs]]` 경로에 있어서 발생한 문제였고 상위 폴더의 `layout.tsx`에 옮겼더니 해결됐다. 또한 라우트에 포함되는 노드들은 `useEffect`로 flag 처리해 처음 한 번만 열리도록 했다. 그렇지 않으면 영원히 닫을 수 없었다. 다 만들고 나니 기존 라이브러리의 `defaultOpen`을 사용한 것과 차이가 없었다. 정말이지 오버엔지니어링이 따로 없었다.
<br/>
