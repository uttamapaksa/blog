...
#### S3 + CloudFront, Access Denied ✅ <small className="text-gray-500">24.02.01.</small>
...
#### HTTPS - HTTP Mixed Content 오류 ✅ <small className="text-gray-500">24.02.01.</small>
HTTPS로 로드된 페이지(프론트엔드)에서 HTTP(백엔드)로 리소스를 요청하는 경우 브라우저가 안전을 이유로 차단한다. 당장 백엔드를 HTTPS로 전환하는 것은 부담이어서 CloudFront를 통해 우회적으로 해결했다. EC2 백엔드를 CloudFront를 통해 HTTPS로 제공하면 프론트엔드와 CloudFront는 HTTPS로, CloudFront와 백엔드는 HTTP로 통신하여 백엔드를 HTTPS로 전환하지 않고도 해결할 수 있다. 대신 백엔드의 보안을 위해 퍼블릭 액세스 차단, CORS, 커스텀 헤더 등을 추가 적용했다.
<br/>
