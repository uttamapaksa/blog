...
#### 넥스트) URL 파라미터 받기기 ✅ <small className="text-gray-500">25.05.11.</small>
...
POST와 PUT은 두 번째 인자로 body(본문)에 데이터를 직접 넘긴다(`axios.post(url, data)`). DELETE는 보통 URL로만 요청하기 때문에 body를 보내려면 두 번째 인자인 옵션 객체에 data 프로퍼티로 넘겨야 한다(`axios.delete(url, {data: sth})`). 
<br/>