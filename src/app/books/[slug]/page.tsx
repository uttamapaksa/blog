...
export default async function BooksDetail({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await getPostBySlug(menu, slug);

  return (
    <main className="mx-auto max-w-6xl px-20 py-8 sm:px-40 sm:py-12">
      <section className="flex justify-between mt-6 text-xs sm:mt-8">
      ...
      <div className="mt-6 h-80">
        <img src={post.thumbnail} alt="post image" className="mx-auto h-full object-cover object-center rounded-md" />
      </div>
